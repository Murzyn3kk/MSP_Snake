...
unsigned int currScore = 0; unsigned int ScoreMultiplier = 1; 
 
unsigned int przedzial = 0; unsigned int DifficultyStep = 5; unsigned int DifficultyStep2 = 15; unsigned int InBetween = 0; 
...
void printDiscr(void); void printScore(void); void printMenu(void); void printCred(void); void Game(void); void Input(void); void InputMenu(void); void Init(void); 
void Send_String(unsigned char characters[], int wybor); void CreateCharacters(); 
 
 
 
 
//Wypisz liczbe 
int WyswietlLiczbe(int liczba, int prev) {   int count = 0; 
...
void printString(char a[]){   if(strlen(a)<4) {      SEND_CHAR('B'); 
    SEND_CHAR('R'); 
    SEND_CHAR('A'); 
    SEND_CHAR('K'); 
...
//GameLoop 
  for (;;){ 
    ...
    if(menu){       
      ...
      printDiscr();   
      menu = true, game = false, score = false, credits = false, discr = false;       clearDisplay(); 
    }else if(score){//Wyswietl wynik 
      printScore(); 
      menu = true, game = false, score = false, credits = false, discr = false;       for(int i = 0; i < 100; i++)       clearDisplay(); 
...
void Send_String(unsigned char characters[], int wybor){ 
  if(wybor == 1) SEND_CMD(DD_RAM_ADDR); 
  else if(wybor == 2) SEND_CMD(DD_RAM_ADDR2);   else return; 
while(*characters != '?') SEND_CHAR(*characters++); 
} 
 
//----------------- utworzenie specjalnych znaków ------------------- 
void CreateCharacters(){ 
  SEND_CMD(CG_RAM_ADDR); 
  for(int i=0; i<8; i++) SEND_CHAR(abovus[i]);   for(int i=0; i<8; i++) SEND_CHAR(cactusus[i]);   for(int i=0; i<8; i++) SEND_CHAR(abovusTop[i]); 
...
//----------------- wypisanie menu ------------------- 
void printDiscr(){   Send_String(opisG, 1);   Send_String(opisD, 2); 
  for(int i = 0; i < 200; i++) Delayx100us(700); 
} 
void printMenu(){   Send_String(menuG, 1); 
  Send_String(menuD, 2); 
} 
void printCred(){   Send_String(creditsA, 1);   Send_String(creditsB, 2); 
  for(int i = 0; i < 50; i++) Delayx100us(700); 
  clearDisplay();   Send_String(creditsC, 1); 
  for(int i = 0; i < 50; i++) Delayx100us(700); 
} 
void printScore() { 
    clearDisplay(); 
...
void InputMenu(void){ 
  ...
  if((P4IN & BIT5)==0){     discr = true;     menu = false; 
  } 
   if((P4IN & BIT6)==0){     score = true; 
    menu = false; 
  } 
  if((P4IN & BIT7)==0){     credits = true;     menu = false; 
  } 
} 
...
void Input(void){ 
  ...
  if(jumpButtonPressed==1 && jumpTime == 0){     line=0; 
    jumpTime=5;     InBetween = 2; 
  } 
...
if(jumpTime>0) jumpTime--; if(jumpTime<=0){ 
line=1; 
jumpButtonPressed = 0; dropButtonPressed = 0; 
